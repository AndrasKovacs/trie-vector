...
-- | Pop the last array from a trie.
...
-- | Pop from a 'Vector' and return only the 'Vector'.
popVector :: forall a. Vector a -> Vector a
popVector (Vector 0#   _     _    _   ) = popError
popVector (Vector size level init tail) = let
    tailSize  = andI# size KEY_MASK
    size'     = size -# 1#
    width     = NODE_WIDTH
