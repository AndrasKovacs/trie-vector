...
a16n n = foldl' A16.snoc A16.empty $ [0..n::Int]
a32n n = foldl' A32.snoc A32.empty $ [0..n::Int]
aun  n = foldl' AU.snoc  AU.empty  $ [0..n::Int]

config = defaultConfig {cfgSamples = ljust 8}


ixa16 a [i]    = a A16.! i 
ixa16 a (i:is) = seq (a A16.! i) (ixa16 a is)
ixa16 _ _      = undefined 

ixau a [i] = a AU.! i 
ixau a (i:is) = seq (a AU.! i) (ixau a is)
ixau _ _ = undefined 
...
moda16 !a [] = a
moda16 !a (i:is) = moda16 (A16.modify a i (const i)) is

modau !a [] = a
modau !a (i:is) = modau (AU.modify a i (const i)) is
...
a16snoc !a [] = a
a16snoc !a (i:is) = a16snoc (A16.snoc a i) is
